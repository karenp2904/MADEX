...
export const Tarjeta = () => {
    ...
    const [isHideBackCard, setIsHide] = useState(false);
    ...
    // Handle CCV update
    const [inputCCVNumber, setInputCCVNumber] = useState(""); // document.getElementById("ccvNumber");
    const [imageCCVNumber, setImageCCVNumber] = useState(""); // document.getElementById("imageCCVNumber");

    const handleInputCCVNumber = (event) => {
    ...
    const [inputCardName, setImageInputCardName] = useState("");
    const [imageCardName, setImageCardName] = useState("");

    const handleInputCardName = (e) => {
        setImageInputCardName(e.target.value);
        setImageCardName(e.target.value);
    }
        ...
        <div className="flex">
            ...
            <div className="flex justify-center items-center w-full lg:w-1/2 lg:pl-8">
                <div className="w-full max-w-sm h-56" style={{ perspective: 1000 }}>
                    <div
                        id="creditCard"
                        className={`${isHideBackCard ? "transform" : ""} relative crediCard cursor-pointer transition-transform duration-500`}
                        style={{ transformStyle: "preserve-3d", transform: isHideBackCard ? `rotateY(-180deg)` : ""}}
